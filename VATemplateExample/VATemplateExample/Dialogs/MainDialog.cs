...
namespace VATemplateExample.Dialogs
{
    ...
    public class MainDialog : ComponentDialog
    {
        ...
        private readonly EscalateDialog _escalateDialog;
        ...
        private async Task<bool> InterruptDialogAsync(DialogContext innerDc, CancellationToken cancellationToken)
        {
            ...
            var activity = innerDc.Context.Activity;
            var userProfile = await _userProfileAccessor.GetAsync(innerDc.Context, () => new UserProfileState(), cancellationToken);
                    ...
                    if (generalScore > 0.5)
                    {
                        switch (generalIntent)
                        {
                            case GeneralLuis.Intent.Cancel:
                                {
                                    ...
                                    // Tell middleware to close escalated conversation if one is active
                                    var evnt = EventFactory.CreateHandoffStatus(innerDc.Context.Activity.Conversation, "closed");
                            ...
                            case GeneralLuis.Intent.Escalate:
                                {
                                    await innerDc.BeginDialogAsync(_escalateDialog.Id, cancellationToken: cancellationToken);
        ...
        {
            var userProfile = await _userProfileAccessor.GetAsync(stepContext.Context, () => new UserProfileState(), cancellationToken);
        ...
        {
            // activity will be null when events flow through RouteStepAsync
