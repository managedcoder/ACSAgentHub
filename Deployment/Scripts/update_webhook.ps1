...
if ($result)
{
  ...
  if ($showCommands.ToLower() -eq "true") { Write-Host ""; write-host "az eventgrid event-subscription update --name ""$eventSubscriptionName"" --endpoint ""$endpoint"" --endpoint-type ""webhook"" --source-resource-id ""/subscriptions/$($azureSubscriptionId)/resourceGroups/$($resourceGroup)/providers/Microsoft.Communication/CommunicationServices/$($acsServiceName)"" --subscription ""$($azureSubscriptionId)"" "}
